...
if [[ ! ${repos[*]} =~ ${CORRAL_rancher_chart_repo} ]]; then
  echo 'Error: `rancher_chart_repo` must be one of ["latest", "alpha", "stable", "prime"]'
  exit 1
fi
...
if [ "$minor_version" -gt 24 ]; then
    ...
    if [[ "$CORRAL_rancher_chart_url" == "https://charts.optimus.rancher.io/server-charts/" ]]; then
    ...
    helm upgrade --install rancher rancher-$CORRAL_rancher_chart_repo/rancher --namespace cattle-system --set hostname=$CORRAL_rancher_host --version=$CORRAL_rancher_version --set proxy=http://$CORRAL_registry_private_ip:3219 --set noProxy=localhost\\,127.0.0.0/8\\,10.0.0.0/8\\,172.0.0.0/8\\,192.168.0.0/16\\,.svc\\,.cluster.local\\,cattle-system.svc\\,169.254.169.254
fi
